  ...
  for (auto arg : args)
  {
    switch (arg.type.type)
    {
      case Type::integer:
        ...
        break;
      case Type::mapstr: {
        AsyncEvent::StrMap *strMap(
            reinterpret_cast<AsyncEvent::StrMap *>(arg_data + arg.offset));
        uint64_t mapfd(strMap->mapfd);
        int32_t arrayIx(static_cast<int32_t>(strMap->arrayIx & 0xffffffff));
        uint64_t strLen(strMap->strLen);

        std::shared_ptr<IMap> map = get_mapstr_map_by_fd(
            static_cast<int32_t>(mapfd & 0xffffffff));
        ...
        std::string_view ourString(&reinterpret_cast<char &>(
                                       str_buff.at(map->value_size_ * cpu_id)),
                                   strLen);

        arg_values.push_back(
            std::make_unique<PrintableString>(std::string(ourString)));
        break;
      }
      case Type::string:
      {
