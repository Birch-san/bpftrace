...
void CodegenLLVM::visit(Call &call)
{
  ...
  else if (call.func == "str")
  {
    ...
    b_.CreateCondBr(b_.CreateICmpNE(str_map,
                                         ...
                                         const std::string &call_name, AsyncAction async_action)
{
  ...
  auto null_ptr = ConstantExpr::getCast(Instruction::IntToPtr,
                                        b_.getInt64(0),
                                        fmt_struct_ptr_ty);
  b_.CreateCondBr(b_.CreateICmpNE(fmt_args, null_ptr, "fmtstrcond"),
                  notzero,
                  zero);
  ...
  for (size_t i=1; i<call.vargs->size(); i++)
  {
    ...
    arg.accept(*this);
    // TODO: eventually operands such as "string backed by map" will have a
    // failure-mode "failed to acquire free map index" in that situation: we
    // will want to abort the print (and log why)
