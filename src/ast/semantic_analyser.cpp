...
void SemanticAnalyser::visit(Call &call)
{
  ...
  else if (call.func == "signal")
  {
    ...
    for (auto &ap : *probe_->attach_points)
    {
      ...
      else if (type == ProbeType::interval || type == ProbeType::software ||
               type == ProbeType::hardware || type == ProbeType::watchpoint)
      {
        error(call.func + " can not be used with \"" + ap->provider +
                  "\" probes",
              call.loc);
  ...
  if (unop.op == Parser::token::MUL)
  {
    if (type.IsCastTy() || type.IsCtxTy())
    {
      if (type.is_pointer)
      {
        ...
        if (k_v == intcasts.end() &&
            bpftrace_.structs_.count(type.cast_type) == 0)
        {
        ...
        if (k_v != intcasts.end())
        {
          auto &v = k_v->second;
          unop.type = SizedType(
              Type::integer, std::get<0>(v), std::get<1>(v), k_v->first);
        }
        else
        {
    ...
    for (AttachPoint *attach_point : *probe_->attach_points)
    {
      ...
      for (auto &match : matches)
      {
        ...
        structs[tracepoint_struct] =
            bpftrace_.structs_[tracepoint_struct].fields;
      }
    }
  }
  else
...
void SemanticAnalyser::visit(AttachPoint &ap)
{
  ...
  else if (ap.provider == "profile")
  {
    ...
    else if (ap.target != "hz" && ap.target != "us" && ap.target != "ms" &&
             ap.target != "s")
  ...
  else if (ap.provider == "interval")
  {
    ...
    else if (ap.target != "ms" && ap.target != "s")
      error(ap.target + " is not an accepted unit of time", ap.loc);
    if (ap.func != "")
    ...
    else
    {
      ...
      for (auto &probeListItem : SW_PROBE_LIST)
      {
        if (ap.target == probeListItem.path ||
            (!probeListItem.alias.empty() && ap.target == probeListItem.alias))
        {
          found = true;
          break;
        }
...
int SemanticAnalyser::create_maps(bool debug)
{
  ...
  for (auto &map_val : map_val_)
  {
    ...
    if (debug)
    {
      bpftrace_.maps_[map_name] = std::make_unique<bpftrace::FakeMap>(map_name,
                                                                      type,
                                                                      key);
  ...
  if (debug)
  {
    if (needs_join_map_)
    {
      // join uses map storage as we'd like to process data larger than can fit
      // on the BPF stack.
      std::string map_ident = "join";
      SizedType type = CreateJoin(bpftrace_.join_argnum_,
                                  bpftrace_.join_argsize_);
      MapKey key;
      bpftrace_.join_map_ = std::make_unique<bpftrace::FakeMap>(map_ident,
                                                                type,
                                                                key);
    }
    if (needs_elapsed_map_)
    {
      ...
      MapKey key;
      bpftrace_.elapsed_map_ = std::make_unique<bpftrace::FakeMap>(map_ident,
                                                                   type,
                                                                   key);
    }
  ...
  else
  {
    ...
    if (needs_elapsed_map_)
    {
      ...
      SizedType type = CreateUInt64();
      MapKey key;
      bpftrace_.elapsed_map_ = std::make_unique<bpftrace::Map>(
          map_ident, type, key, 1);
