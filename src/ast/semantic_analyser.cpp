  ...
  if (call.func == "hist") {
    ...
    if (check_varargs(call, 1, 2)) {
      ...
      if (is_final_pass() && call.vargs->size() > 1) {
        ...
        // that lets you specify the max str size of a dynamic-length string
        // (i.e. let people configure on per-string basis rather than globally)
      }
...
int SemanticAnalyser::create_maps(bool debug)
{
  ...
  else
  {
    if (needs_join_map_)
    {
      ...
      bpftrace_.join_map_ = std::make_unique<bpftrace::Map>(
          map_ident, type, key, 1, type.size);
  ...
  if (needs_fmtstr_map_)
  {
    ...
    if (debug)
      ...
      bpftrace_.fmtstr_map_ = std::make_unique<bpftrace::Map>(
          map_ident, type, key, 1, type.size);
      bpftrace_.fmtstr_map_zero_ = calloc(1,
                                          max_fmtstr_args_size_ +
                                              sizeof(size_t));
    }
  ...
  for (auto *strCall : str_calls_)
  {
    ...
    {
      map = std::make_shared<bpftrace::Map>(
          map_ident, type, key, bufferSize, value_size);
    }
    ...
    else
    {
      ...
      std::unique_ptr<std::byte, StrCall::StrMapState::ZeroesDeleter>
          zeroesForClearingMap(
              (std::byte *)(std::calloc(value_size, sizeof(std::byte))));
      strCall->state.emplace(std::move(map), std::move(zeroesForClearingMap));
    }
    strCallIx++;
  }
